# -----------------------------------------------------------------------------
# Author: Matt Martz, Ryo Nakagami
# Revised: 2025-11-14
# Script: speedtest.py
# Description:
#   This script provides a command-line interface for testing internet bandwidth
#   using speedtest.net. It retrieves server configurations, selects the best
#   server based on latency, and performs download and upload speed tests.
#
#   Steps:
#     1. Parse command-line arguments to configure the test.
...
#   - Optional dependencies: json/simplejson, SSL support.
#
...
class SpeedtestHTTPConnection(HTTPConnection):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self._tunnel = None  # Define _tunnel attribute to avoid Pylance errors
...
class SpeedtestHTTPSHandler(AbstractHTTPHandler):
    ...
    def __init__(self, debuglevel=0, context=None, source_address=None, timeout=10):
        AbstractHTTPHandler.__init__(self, debuglevel)
        self._context = context if context else ssl.create_default_context()
        self.source_address = source_address
        self.timeout = timeout
    ...
    def __init__(self, response):
        ...
        if not gzip:
            raise SpeedtestHTTPError(
                "HTTP response body is gzip encoded, but gzip support is not available"
            )
        IO = BytesIO or StringIO
        self.io = IO()
        while 1:
            if response is None:
                raise SpeedtestHTTPError("Response object is None.")
        ...
        try:
            if (timeit.default_timer() - self.starttime) <= self.timeout:
                ...
                while (
                    ...
                    if f is None:
                        raise SpeedtestHTTPError("File object is None.")
...
class SpeedtestResults(object):
    ...
    def __init__(
        ...
        self._share = None
        self.timestamp = (
            "%sZ" % datetime.datetime.now(datetime.timezone.utc).isoformat()
        )
    ...
    def share(self):
        ...
        if f is None:
            raise ShareResultsConnectFailure("File object is None.")
        response = f.read()
        code = f.code
        f.close()
...
class Speedtest(object):
    ...
    def get_config(self):
        ...
        while 1:
            try:
                if stream is None:
                    raise ConfigRetrievalError("Stream object is None.")
        ...
        if uh is None or not hasattr(uh, "code") or int(uh.code) != 200:
            return None
